...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    vals, cnt = np.unique(feature_vector, return_counts=True)
    thresholds = np.delete(vals / 2 + np.append((vals / 2)[1:], 0), -1)

    left = cnt.cumsum()[:-1]
    right = feature_vector.shape[0] - left

    vals, cnt = np.unique(np.sort(feature_vector), return_index=True)
    target = target_vector[np.argsort(feature_vector)]

    p1_l = target.cumsum()[cnt[1:] - 1] / left
    p0_l = 1 - p1_l
    h_l = 1 - (p1_l ** 2) - (p0_l ** 2)

    p1_r = (target_vector.sum() - target.cumsum()[cnt[1:] - 1]) / right
    p0_r = 1 - p1_r
    h_r = 1 - (p1_r ** 2) - (p0_r ** 2)
...
class DecisionTree(ClassifierMixin, BaseEstimator):
    def __init__(self, feature_types, max_depth=15, min_samples_split=4, min_samples_leaf=1):
